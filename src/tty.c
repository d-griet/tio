...
bool tty_configure_from_fd(int fd)
{
    int status;
    speed_t baudrate;

    /* Use existing port state as baseline so we don't start from a
     * zeroed struct on platforms (e.g. Windows/MSYS2) where the driver
